...
ggplot(kmeansPCA, aes(x = kmeansPCA$scores[,1], y = kmeansPCA$scores[,2], color = reducedGroupData$Class)) + geom_text(label = Ksol1$cluster) + ggtitle("PCA for K-Means five cluster solution")
```
...
ggplot(kmeansPCA2, aes(x = kmeansPCA2$scores[,1], y = kmeansPCA2$scores[,2], color = reducedGroupData$Class)) + geom_text(label = Ksol2$cluster) + ggtitle("PCA for K-Means three cluster solution")
```
...
ggplot(singlePCA, aes(x = kmeansPCA$scores[,1], y = singlePCA$scores[,2], color = reducedGroupData$Class)) + geom_text(label = singleSol) + ggtitle("PCA for Single Linkage five cluster solution")
```
...
ggplot(singlePCA1, aes(x = singlePCA1$scores[,1], y = singlePCA1$scores[,2], color = reducedGroupData$Class)) + geom_text(label = singleSol1) + ggtitle("PCA for Single Linkage three cluster solution")
```
...
ggplot(completePCA, aes(x = completePCA$scores[,1], y = completePCA$scores[,2], color = reducedGroupData$Class)) + geom_text(label = compSol) + ggtitle("PCA for Complete Linkage five cluster solution")
```
...
ggplot(compPCA1, aes(x = compPCA1$scores[,1], y = compPCA1$scores[,2], color = reducedGroupData$Class)) + geom_text(label = compSol1) + ggtitle("PCA for Complete Linkage three cluster solution")
```
...
ggplot(wardPCA1, aes(x = wardPCA1$scores[,1], y = wardPCA1$scores[,2], color = reducedGroupData$Class)) + geom_text(label = wardSol) + ggtitle("PCA for Ward's method five cluster solution")
```
...
protein_heatmap <- heatmap(protein_matrix, Rowv=NA, Colv=NA, col = cm.colors(256), scale="column", main="HeatMap of all Protein Variables")
```

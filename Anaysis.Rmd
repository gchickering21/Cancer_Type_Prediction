...
```{r}
hagnes <- agnes(reducedGroupData[, -c(56)], diss = FALSE, stand = TRUE, method = "ward")
plot(hagnes, which = 2)
plot(hagnes, which = 1) 
```
...
```{r}
compare <- clValid(reducedGroupData[,-c(56)], 3:8, clMethods = c("agnes", "kmeans", "diana", "pam", "clara"), metric = "euclidean", method = "complete")
```
